...
#
# Kosmos | Bot Discord français universel
#
# Basé sur Redbot par Twentysix26. Plus d'infos sur https://github.com/Twentysix26/
#
#
# red.py et cogs/utils/checks.py contiennent du contenu édité par Rapptz.
#                 https://github.com/Rapptz/RoboDanny/
#

description = "Kosmos - Bot Discord universel et modulaire"
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        print("Connexion réalisée avec succès. ({}ms)\n".format(login_time))

        owner = await set_bot_owner()

        print("=================")
        print("Kosmos - Bot Discord")
        print("=================")
        print(str(bot.user))
        print("\nStats:")
        ...
        print("Propriétaire: " + str(owner))
        print("{}/{} extensions avec {} commandes".format(
            len(bot.cogs), total_cogs, len(bot.commands)))
        print("=================")

        if bot.settings.token and not bot.settings.self_bot:
            print("\nUtilisez cet URL pour lui faire rejoindre votre serveur:")
            url = await get_oauth_url()
            bot.oauth_url = url
            print(url)

        print("Gardez votre bot à jour avec l'option correspondante dans le menu du launcher.")
    ...
    async def on_command_error(error, ctx):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            no_dms = "Impossible d'envoyer un message à cet utilisateur."
            is_help_cmd = ctx.command.qualified_name == "help"
            is_forbidden = isinstance(error.original, discord.Forbidden)
            if is_help_cmd and is_forbidden and error.original.text == no_dms:
                msg = ("Je n'ai pas pu vous envoyer le message d'aide en MP. Avez-vous osé me bloquer ?")
                await bot.send_message(channel, msg)
                return

            bot.logger.exception("Erreur dans la commande '{}'".format(
                ctx.command.qualified_name), exc_info=error.original)
            message = ("Erreur dans '{}'. Consultez la console pour plus d'informations."
                       "".format(ctx.command.qualified_name))
            log = ("Erreur dans la commande '{}'\n"
                   "".format(ctx.command.qualified_name))
            ...
            await bot.send_message(channel, "Cette commande est en cooldown. Reesayez dans {:.2f}s"
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Kosmos - Configuration d'origine\n")
        print("Si ce n'est pas déjà fait, créez un compte BOT (en):\n"
              "https://twentysix26.github.io/Red-Docs/red_guide_bot_accounts/"
              "#creating-a-new-bot-account")
        print("et copiez le token comme indiqué.")
    ...
    if not settings.login_credentials:
        ...
        while settings.token is None and settings.email is None:
            ...
            if "@" not in choice and len(choice) >= 50:  # Assuming token
                ...
                print("Ce token n'est pas valide.")
        settings.save_settings()

    if not settings.prefixes:
        print("\nChoisissez le préfixe qui vient se placer en début de commande. Il sera toujours possible de le "
              "changer plus tard :")
        confirmation = False
        while confirmation is False:
            new_prefix = ensure_reply("\nPrefix> ").strip()
            print("\nVoulez-vous {0} comme préfixe ?\nLes commandes ressembleront à ça: {0}help"
                  "\nTapez 'yes' pour confirmer.".format(
                      new_prefix))
    ...
    if first_run:
        print("\nEntrez le nom du rôle dédié aux administrateurs")
        print("Laissez vide si vous voulez le rôle par défaut (Administrateur)")
        settings.default_admin = input("\nAdmin role> ")
        if settings.default_admin == "":
            settings.default_admin = "Administrateur"
        settings.save_settings()

        print("\nEntrez le nom du rôle dédié aux modérateurs")
        print("Laissez vide pour le rôle par défaut (Modérateur)")
        settings.default_mod = input("\nModerator role> ")
        if settings.default_mod == "":
            settings.default_mod = "Modérateur"
        settings.save_settings()

        print("\nLa configuration est terminée. Laissez cette fenêtre ouverte pour que le bot soit actif.\n"
              "Cette fenêtre passe désormais en LECTURE SEULEMENT\n"
              "Voici un guide en anglais qui pourrait vous aider sur la base du bot:\n"
              "https://twentysix26.github.io/Red-Docs/red_getting_started/\n"
              "Apputez sur ENTRER pour continuer")
        input("\n")
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("Le module 'Owner' n'est pas disponible. J'en ai besoin pour fonctionner correctement. "
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not bot.settings.no_prompt:
            choice = input("Les clefs de connexion sont invalide... Essayez de redémarrer ce script. "
